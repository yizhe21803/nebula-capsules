...
assert.match(moduleSource, /valueNoise2D/);
assert.match(moduleSource, /fbm2D/);
assert.match(moduleSource, /edgeEnvelope/);
assert.match(moduleSource, /drawReferenceFlow/);
assert.match(moduleSource, /drawColorClouds/);
assert.match(moduleSource, /drawBlurredLayer/);
assert.match(moduleSource, /pointerdown/);
assert.match(moduleSource, /pointermove/);
assert.match(moduleSource, /loadRate \* delta/);
assert.doesNotMatch(moduleSource, /setLineDash|drawBackwashStrands|crestBurst/);
...
console.log('Progress capsule reference-motion isolation tests passed.');
